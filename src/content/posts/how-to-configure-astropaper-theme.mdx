...
| `dynamicOgImage`   | Generate a dynamic OG image per post when no `ogImage` is specified in frontmatter. Defaults to `true`. See the [trade-off](/posts/dynamic-og-image-generation-in-astropaper-blog-posts/#trade-off) for details. |
...
- [Adding new posts](/posts/adding-new-post/) — frontmatter reference and file conventions.
