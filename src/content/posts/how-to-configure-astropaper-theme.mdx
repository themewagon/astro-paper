...
| `dynamicOgImage`   | Generate a dynamic OG image per post when no `ogImage` is specified in frontmatter. Defaults to `true`. See the [trade-off](/astro-paper/posts/dynamic-og-image-generation-in-astropaper-blog-posts/#trade-off) for details. |
...
- [Customizing AstroPaper theme color schemes](/astro-paper/posts/customizing-astropaper-theme-color-schemes/) — change or add color schemes via `src/styles/theme.css`.
- [Adding new posts](/astro-paper/posts/adding-new-post/) — frontmatter reference and file conventions.
